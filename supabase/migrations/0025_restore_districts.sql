-- الاستعادة تعيد الأحياء وشروطها أيضًا.
--
-- جدولان جديدان (`districts` و`district_rules`) دخلا اللقطة في ٠٢٤، ولو
-- بقيا خارج الاستعادة لعادت النسخة بلا حدودٍ ولا شروط — أي أنّ فحص الموقع
-- يبدأ من الصفر بعد كلّ استعادة، والحجوزات القديمة تُفسَّر بغير ما حُسبت به.
--
-- والترتيب مقصود: الحدود قبل الشروط، فالشرط يشير إلى الحيّ بمفتاحٍ أجنبيّ.
-- وشرطٌ لحيٍّ غير موجود يُترك بدل أن يُسقط الاستعادة كلَّها.
...
declare
  ...
  n_act int := 0; n_scan int := 0; n_dis int := 0; n_drul int := 0;
  v_ver text;
begin
  ...
  -- الحدود قبل الشروط.
  with src as (select * from jsonb_populate_recordset(null::public.districts,
                                 coalesce(p_data->'districts', '[]'::jsonb)))
  , ins as (insert into public.districts select * from src
            on conflict (id) do nothing returning 1)
  select count(*) into n_dis from ins;

  with src as (select * from jsonb_populate_recordset(null::public.district_rules,
  ...
  select count(*) into n_drul from ins;
  ...
  return jsonb_build_object(
    ...
    'districts', n_dis, 'district_rules', n_drul,
